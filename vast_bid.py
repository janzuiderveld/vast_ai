...
if args.bidding_tactic == 'cheap':
    ...
    vast_cmd = './vast search offers "num_gpus == 1" --raw -o "dlperf-"'   
elif args.bidding_tactic == 'auto1':
    vast_cmd = './vast search offers "num_gpus == 1" --raw'   
...
for i in range(10):
    print(top_sellers[i]["dph_base"], top_sellers[i]["min_bid"], top_sellers[i]["dlperf"], top_sellers[i]["gpu_ram"])
...
vast_book_cmd = f"./vast create instance {top_id} --price {bid} --image pytorch/pytorch --disk 15 --onstart startup_scripts/{args.project_name}.sh"
